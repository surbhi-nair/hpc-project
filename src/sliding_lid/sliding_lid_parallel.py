...
if __name__ == "__main__":
    ...
    # Define lid velocity
    lid_velocity = torch.tensor([args.velocity, 0.], dtype=torch.float32, device=DEVICE)
    ...
    density = torch.ones((proc_dim_x, proc_dim_y), dtype=torch.float32, device=DEVICE)
    # Initialize velocity u(0) = 0.0 at time t=0
    velocity = torch.zeros((2, proc_dim_x, proc_dim_y), dtype=torch.float32, device=DEVICE)
    # Initialize proba density to equilibrium
    proba_density = lbm.calculate_equilibrium_distro(density, velocity)
    proba_density = proba_density.to(DEVICE)
    ...
    if not benchmarking:
        save_mpiio(cartcomm, 'ux.npy', velocity[0, x_start:x_end, y_start:y_end].cpu().numpy())
        save_mpiio(cartcomm, 'uy.npy', velocity[1, x_start:x_end, y_start:y_end].cpu().numpy())
